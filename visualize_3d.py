...
def create_3d_visualization(embeddings_3d, class_labels, modalities, class_names, 
    ...
    for class_idx, class_name in enumerate(class_names):
        ...
        if raman_mask.any():
            fig.add_trace(
                go.Scatter3d(
                    x=embeddings_3d[raman_mask, 0],
                    y=embeddings_3d[raman_mask, 1],
                    z=embeddings_3d[raman_mask, 2],
                    mode='markers',
                    marker=dict(
                        size=4,
        ...
        if gc_mask.any():
            fig.add_trace(
                go.Scatter3d(
                    x=embeddings_3d[gc_mask, 0],
                    y=embeddings_3d[gc_mask, 1],
                    z=embeddings_3d[gc_mask, 2],
                    mode='markers',
                    marker=dict(
                        size=4,
                        color=colors_hex[class_idx],
                    ...
                    legendgroup=f'class_{class_idx}',
                    hovertemplate=f'<b>{class_name} (GC)</b><br>' +
                                 'X: %{x:.2f}<br>Y: %{y:.2f}<br>Z: %{z:.2f}<extra></extra>'
                ),
                row=1, col=1
            )
