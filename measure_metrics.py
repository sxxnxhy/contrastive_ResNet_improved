...
from model import SpectralEncoder1D
...
def main():
    ...
    parser.add_argument('--checkpoint', type=str, default='models/ContrastiveModel_ResNet.pth')
    ...
    # Build encoder and optional classifier
    encoder = SpectralEncoder1D(in_channels=1, hidden_dim=config.HIDDEN_DIM, embed_dim=config.EMBEDDING_DIM)
    encoder.to(device)

    num_classes = len(classes)
    classifier = nn.Linear(config.EMBEDDING_DIM, num_classes)
    ...
    # Try to load states
    try:
        # If model_state contains classifier sub-dict, load it
        if isinstance(model_state, dict) and 'classifier' in model_state and isinstance(model_state['classifier'], dict):
            try:
        ...
        # Now load encoder weights
        # model_state may be nested
                ...
                if isinstance(model_state, dict):
                    for k, v in model_state.items():
                        if isinstance(v, dict):
                            try:
                                encoder.load_state_dict(v)
                                loaded = True
                                break
                            except Exception:
                                continue
                if not loaded:
                    print("Warning: could not load encoder weights cleanly; proceeding with init weights")
